...
app.post('/teacher/create-test', async (req, res) => {
  if (req.session.user && req.session.user.role === 'teacher') {
    const { subject, duration, questionCount } = req.body;
    const testId = crypto.randomBytes(3).toString('hex').toUpperCase();
    ...
    try {
      const newTest = {
        ...
        questions: [],
        isLive: false,  // Initially set to false
...
app.get('/student/live-tests', async (req, res) => {
  if (req.session.user && req.session.user.role === 'student') {
    try {
      ...
      res.json({ success: true, tests: liveTests });
    } catch (error) {
...
app.get('/teacher/student-answers/:testId/:studentName', async (req, res) => {
  if (req.session.user && req.session.user.role === 'teacher') {
    const { testId, studentName } = req.params;
    try {
      const test = await db.collection('tests').findOne({ _id: testId });
...
app.post('/teacher/update-test/:testId', async (req, res) => {
  if (req.session.user && req.session.user.role === 'teacher') {
    ...
    try {
      await db.collection('tests').updateOne(
        { _id: testId },
        { $set: { subject, duration, questionCount: parseInt(questionCount), questions } }
      );
      res.json({ success: true });
    } catch (error) {
...
app.post('/teacher/send-score', async (req, res) => {
  if (req.session.user && req.session.user.role === 'teacher') {
    ...
    try {
      ...
      if (student && student.testScores && student.testScores[testId]) {
        await db.collection('students').updateOne(
          { username: studentName },
  ...
  if (req.session.user && req.session.user.role === 'student') {
    ...
    try {
      ...
      const test = await db.collection('tests').findOne({
        $or: [
          { _id: testId, code: code, isLive: true },
          { code: code, isLive: true }
        ]
      });
  ...
  if (req.session.user && req.session.user.role === 'student') {
    ...
    try {
      ...
      if (test) {
        ...
        res.json({ success: false, message: 'Test not found or not active' });
      }
    } catch (error) {
      console.error('Error starting test:', error);
      res.status(500).json({ success: false, message: 'An error occurred while starting the test' });
    }
  } else {
    res.status(403).json({ success: false, message: 'Unauthorized' });
  }
